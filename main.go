...
func DrawingLoop(filename string, todayWinEnabled bool, debug bool) {
	...
	for {
		...
		if updateEvents {
			...
			if debug {
				statusMessage = fmt.Sprintf("Remind took %fs", time.Now().Sub(start).Seconds())
			}
			updateEvents = false
		}
		...
		case 9:
			if activeWin == CALENDAR_WIN {
				activeWin = EVENTS_WIN
			} else if activeWin == EVENTS_WIN {
				if todayWinEnabled {
					activeWin = TODAY_WIN
				} else {
					activeWin = CALENDAR_WIN
				}
			} else if activeWin == TODAY_WIN {
				activeWin = CALENDAR_WIN
			}
...
func drawEvents(
	...
	for count := 0; ; count++ {
		...
		if row > yOffset {
			Mvwhline(win, y+1+row-yOffset, x+1, ACS_HLINE, w-2)
		}
...
func updateCalendar(win *Window, y int, x int, active bool, ys YearStructure, d Date, today Date, events map[string][]Event) {
	...
	for j := daysInMonthPrev - wdStart + 1; j <= daysInMonthPrev; j++ {
		...
		// add events
		year, month := SubtractMonth(d.Year, d.Month)
		if _, ok := events[NumericString(year, month, j)]; ok {
			eventsIndex[i] = true
		}

		if today.Day == j && today.Month == month && today.Year == year {
			todayIndex = i
		}
	...
	for j := 1; j <= daysInMonth; j++ {
		...
		// add events
		if _, ok := events[NumericString(d.Year, d.Month, j)]; ok {
			eventsIndex[i] = true
		}

		if today.Day == j && today.Month == d.Month && today.Year == d.Year {
			todayIndex = i
		}
